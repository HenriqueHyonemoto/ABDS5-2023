...
--1)Desenvolva uma função que receba o código do cliente como parâmetro e devolva o nome e o endereço concatenados. 
CREATE OR REPLACE FUNCTION f_NomeEndereco (codcliente integer) RETURNS text
AS $$
...
select f_NomeEndereco (720)
--2)Projete uma função que informado o código do cliente por parâmetro, encontre o valor total das compras desse cliente. Como retorno, 
--a função deve informar o nome do cliente concatenado com o valor da compra. Você deverá usar as tabelas cliente, pedido, item_pedido e produto. 
create or replace function f_total(codcli numeric) returns text
as
$$
declare
...
totalp numeric;
begin
select cli.nome_cliente, sum(prod.valor_venda*itpd.quantidade) "valor total" into nomecli,totalp from cliente cli
inner join pedido ped
on ped.codigo_cliente = cli.codigo_cliente
inner join item_pedido itpd
on itpd.num_pedido = ped.num_pedido
inner join produto prod
on prod.codigo_produto = itpd.codigo_produto
where cli.codigo_cliente = codcli
group by cli.nome_cliente;
return nomecli ||' Gastou: R$' ||totalp ||' Em produtos' ;
end;
$$
language plpgsql;
...
--3)Projete uma função que passado o código do cliente, retorne as informações nome, endereco, cidade, uf e cep em forma de registro. Implemente na função o controle, por meio de Raise, de cliente não encontrado (Registro)
create or replace function f_clinfos(codcli numeric)returns record
as
$$
declare
clinfosreg record;
begin
select nome_cliente,endereco,cidade,uf into clinfosreg from cliente where codigo_cliente = codcli;
if not found then
raise 'Não encontrado' using ERRCODE ='ERR01';
end if;
return clinfosreg;
end;
$$
...
select * from f_clinfos(720) as (nome_cliente varchar, endereco varchar, cidade varchar, uf char(2));



--4)Desenvolva uma função para atualizar os valores dos produtos. 
--A atualização será seletiva. Para produtos cuja unidade seja ’G’, 
--terão reajuste de 8%; para produtos cuja unidade seja ’M’, terão reajuste negativo de 5%; 
--as demais unidades não sofrerão reajuste. A função não terá parâmetro de entrada. 
--O retorno dela serão os códigos do produtos, as descrições, unidade, os valores antigos e os valores novos.
select * from produto;
...
begin
for regprod in
	select codigo_produto, descricao, unidade, valor_venda "valor_antigo", valor_venda "valor_novo"
	from produto
loop
	if (regprod.unidade = 'G') then
		regprod.valor_novo = regprod.valor_novo*1.08;
		update produto
		set valor_venda = regprod.valor_novo
...
$$
language plpgsql;
select * from f_atualizaproduto() as (codigo_produto numeric, descricao varchar, unidade char(3), valor_antigo numeric, valor_novo numeric)
select * from produto;